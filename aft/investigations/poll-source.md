...
**(a) EXTERNAL HARDWARE.** The `80`/`81` polls are generated by an external SAS
**host/poller that reaches the cabinet over the physical COM11 serial line through
the USB multiplexer** (`MUX/SAS (COM:11)` = `VE MULTIPLEXER 4004145 SI 2CH`,
`USB\VID_0483&PID_5740\206139555241`). `CommCtrlSAS.exe` only **bridges** those
serial polls onto loopback TCP; `GoldClub.Aurum.Services` is the SAS **slave**
that answers `00`. Nothing on the PC originates the polls.

- **(b) internal process - RULED OUT.** No PC process can produce the polls:
